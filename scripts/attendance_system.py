...
# ===== 配置区域 =====
YOLO_MODEL_PATH = "D:\AI_projects\\face_attendance_yolo11\self_version_wyx\\runs\detect\\runs\\face\exp14\weights\\best.pt"
...
def load_models():
    ...
    # 获取所有注册人员名单（去重）
    registered_names = sorted(face_db.keys())
    ...
    return yolo_model, known_names, known_encodings, registered_names
...
def main():
    ...
    # 加载模型
    yolo_model, known_names, known_encodings, registered_names = load_models()
    ...
    # ===== 统计未签到人员 =====
    signed_names = set(attendance_record.keys())
    all_names = set(registered_names)
    absent_names = all_names - signed_names

    print(f"\n签到统计：")
    print(f"已签到人数: {len(signed_names)}")
    if signed_names:
        print("已签到人员：")
        for name in sorted(signed_names):
            print(f"  ✓ {name}")

    print(f"未签到人数: {len(absent_names)}")
    if absent_names:
        print("未签到人员：")
